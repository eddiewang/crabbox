...
- Added protected AWS Linux and Windows image-candidate publication with versioned recipes, destructive-state scrubbing, strict source/architecture/OS proof, immutable OCI evidence bundles, keyless signing, explicit candidate-only smoke proof, and a dedicated non-admin coordinator credential limited to candidate-owned leases, runs, and images.
