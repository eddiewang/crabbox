...
operator compatibility. Fast Snapshot Restore, catalog retirement, and other
legacy image actions still require `broker.adminToken` or
`CRABBOX_COORDINATOR_ADMIN_TOKEN`.

Protected AWS default promotion reads `CRABBOX_COORDINATOR_PROMOTION_TOKEN`
from the environment and calls the dedicated `/v1/image-promotions` route. The
Worker validates candidate and promotion credentials against the independent
`CRABBOX_IMAGE_CANDIDATE_TOKEN` and `CRABBOX_IMAGE_PROMOTION_TOKEN`; admin,
shared, browser-login, and runtime-adapter credentials cannot authorize either
protected scope. Neither protected credential is accepted as a command-line
flag.
