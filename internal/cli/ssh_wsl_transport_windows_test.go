...
func runFakeWSLTransport(args []string) int {
	...
	case args[1] == "sh" && args[2] == "-c" && len(args) == 7:
		...
		if os.Mkdir(dir, 0o700) != nil || os.WriteFile(filepath.Join(dir, "script.sh"), script, 0o600) != nil {
			return 94
		}
		return 0
	case args[1] == "bash" && len(args) == 3:
		script, err := os.ReadFile(hostPath(args[2]))
		input, inputErr := io.ReadAll(os.Stdin)
		if err != nil || inputErr != nil {
			return 96
		}
		fmt.Printf("script=%x input=%x\n", sha256.Sum256(script), sha256.Sum256(input))
		...
		return 0
	case args[1] == "rm" && len(args) == 5:
		if mode == "cleanup-fail" {
			return 41
		}
		if os.RemoveAll(hostPath(args[4])) != nil {
			return 98
		}
		return 0
	default:
...
func TestWSL2TransportStagesWithoutWindowsAutomount(t *testing.T) {
	...
	for _, path := range []string{filepath.Join(binDir, "wsl.exe"), filepath.Join(openSSHDir, "ssh.exe")} {
	...
	run := func(command string, input []byte) (string, string, error) {
		...
		mustWriteTestFile(t, scriptPath, decodePowerShellCommand(t, command))
	...
	script, workload := []byte("printf exact-script"), []byte("workload\x00stdin\n")
	stdout, stderr, err := run(wsl2CommandWithWaitTimeout(string(script), 0), workload)
	requireFakeWSL(t, err == nil && strings.Contains(stderr, "wsl-stderr-marker"), "embedded transport err=%v stdout=%q stderr=%q", err, stdout, stderr)
	for _, want := range []string{fmt.Sprintf("script=%x", sha256.Sum256(script)), fmt.Sprintf("input=%x", sha256.Sum256(workload))} {
		requireFakeWSL(t, strings.Contains(stdout, want), "stdout=%q missing %q", stdout, want)
	}
	assertEmpty()
	_, _, err = run(wsl2CommandWithWaitTimeout("exit-23", 5*time.Second), nil)
	requireFakeWSL(t, exitCode(err) == 23, "exit=%d err=%v want 23", exitCode(err), err)
	_, stderr, err = run(wsl2CommandWithWaitTimeout("sleep-for-timeout", 250*time.Millisecond), nil)
	requireFakeWSL(t, err != nil && strings.Contains(stderr, "timed out"), "timeout err=%v stderr=%q", err, stderr)
	control := []byte("control-script")
	stdout, _, err = run(wsl2StdinScriptCommandWithWaitTimeout(len(control), 5*time.Second), control)
	requireFakeWSL(t, err == nil && strings.Contains(stdout, fmt.Sprintf("input=%x", sha256.Sum256(nil))), "control transport err=%v stdout=%q", err, stdout)
	assertEmpty()
	t.Setenv("CRABBOX_FAKE_WSL_MODE", "stage-fail")
	_, _, err = run(wsl2CommandWithWaitTimeout("stage-fail", 0), nil)
	requireFakeWSL(t, err != nil, "staging failure succeeded")
	assertEmpty()
	t.Setenv("CRABBOX_FAKE_WSL_MODE", "collision")
	_, _, err = run(wsl2CommandWithWaitTimeout("collision", 0), nil)
